...
parser.add_argument('-r', '--region', dest='region',
                    help='Region (us, ca, uk, etc.)', default='us')
parser.add_argument('-a', '--app', dest='game_id', help='Game ID')
parser.add_argument('-k', '--key', dest='key', help='PushBullet API Key')
parser.add_argument('--use-keybin', dest='use_keybin',
                    help="Use Keybin as Data Store", action="store_true")
parser.add_argument('--keybin-host', dest='keybin_host',
                    help="Keybin endpoint (with http, e.g. http://keybin.com/")
...
def get_last_saved_prices(use_keybin=False):
    if not use_keybin:
        ...
        try:
            content = keybin_client.get_key_value('steam_prices')
        except keybin_client.KeybinError as e:
            print e.message
            content = None
...
def write_current_prices(prices, use_keybin=False):
    ...
    if not use_keybin:
        ...
        try:
            keybin_client.store_key_value('steam_prices', json_prices)
        except keybin_client.KeybinError as e:
            print e.message
...
def main():
    ...
    if use_keybin:
        keybin_client.init(args.keybin_host, args.keybin_user,
                           args.keybin_pass, client_id='steam_price')
